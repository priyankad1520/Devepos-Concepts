1. how do check os in your system: `uname` : o/p:Linux. `cat /etc/os-release` this command shows all detail about os

Docker file name should be `Dockerfile`: just you this command to build the image `docker build -t name_what_you_want_as_an_repo/image:tag/version1 .` why we use "." at the end beacuse tell to the dockerdeamon the dockerfile is in the current directory/location which where i pass the this build commad. 

Incase our dockerfile name is different like docker1 to build this file you should use this commad `docker build -t myrepo:1 -f docker1 .` use -f to if the docker file name is not Dockerfile instaed of other name.

## **Dockerfile – all important instructions in order**
#### 1. FROM: Defines the base image (starting point). where your appication should be host. on ubuntu from call the ubuntu base image to host the applictaion
Why: Every image needs a foundation (like OS or runtime) 
* `FROM ubuntu:20.04` → Base image

```text id="bi1"
A base image is used to provide the runtime environment, operating system, libraries, and dependencies required to run an application inside a container.
Base image provides runtime environment and filesystem,
while Host OS provides the kernel required to run containers.
```

Examples:

* `python:3.11` → Provides Python runtime: Linux environment, Python installed,  pip package manager, Required libraries. So application can run immediately.
* `node:18` → Provides Node.js runtime
* `ubuntu` → Provides Linux OS environment

```text id="bi4"
Base Image = Foundation + Runtime Environment for container
```
Base Image = Foundation + Runtime Environment for container
**“Foundation”** means the basic platform and components required before your application can run inside the container.
...
| Component                   | Purpose                     |
| --------------------------- | --------------------------- |
| Operating System Filesystem not OS. | Basic Linux file structure  |
| OS provide kernal/Host os kenal to run docker| `/bin`, `/etc`, `/usr`
| System Libraries            | Required shared libraries   |
| Runtime                     | Python, Java, Node.js, etc. |
| Package Managers            | apt, yum, pip, npm          |
| Shell/Utilities             | bash, sh, core commands     |
| Environment Setup           | Basic runtime configuration |

👉 Now you can install applications on top of it.

---

# 🔥 Example 2 → Python Base Image
...
👉 Your Python app can directly run.
...
```text id="fd3"
Base Image = House Foundation
Application = Furniture/Rooms
```
...
```text id="fd5"
Foundation = Basic operating system environment and runtime setup required to run applications inside the container.
```
---
...
- An environment variable is a dynamic key-value setting used to store configuration data like usernames, passwords, ports, paths, or API keys for applications and systems.
- Applications need dynamic values while running. Example: Database host, API URL, Port number,Username Instead of hardcoding them in code.
...
Why: Used to install necessary(means what we need to run this application instead of downoling all) packages/ software or setup environment and dependencies. To prepare the image
...
---
#### 12. CMD: when container start running automaticly run/start the application because we do not alway pass command to run an application

it provides Default command/argumnent to run when container starts. The thing is when you pass an argument when you run the docker it will ovveride the argumnet example: if i write cmd ["echo", "hello"] but when i run this command `docker run -it c1:1 echo "hi" the ouput is hi not hello so this is call override.
...
---
#### 13. ENTRYPOINT: Sets main command (like fixed executable). in entrypoint the thing is not override it is just append the argument.
