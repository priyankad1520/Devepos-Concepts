...
`docker run -it imagename:tag` used to run image inside the container. -it meanes interative node/shell/terminal like when we go to run an comment inside the commad we use exce right like that. if we use -d along with this commad it is detach mode we are in the same terminal it does not go inside the container it stay in same host.

if what exit from the contain we use 'crtl+p+q: for safe exit and container is in active state ' if in case we use use 'exit: the running container also move to exit status'

when we want to change the name to our convient use this command: `docker run -it -d --name c1 imagename`

if you what to go inside the container use `docker attach containername/id` it is only work for os images like linux/ubuntu. but it does noy work on application images

application images are nginx,apche/httpd. this things run in bacground. if you run this `docker run -it inmagename` command for application image container it will run .sh file and display logs so to avoid this use `-d`. then you want to go inside this container use `docker exec -it containername /bin/bash` here why i use /bin/bash use the console inside this container. when we use this except in commad clounm /bin/bash any other thing comes like entypoint use /bin/bash
...
how to expose your application : using port mapping. `docker run -it -d --name expose -p 8082:80. 8082 is host port in your security group you should allow it in an any rage. 80: is container port here this port number is mentioned in image or dockerfile in expose line.

use `docker run --help` to look into more arguments.

4. docker ps: Lists running containers on the host machine.

5. docker stop: Stops running container. and docker kill differnece docker stop is gracfull stop kill is forcefully terminated like how we off the laptop closing all tabs is stop. when we directly shutdown the system without close any tabs. 

`docker ps -q` use to display only container id. if you want to kill or stop all container at once use this commad: `docker stop/kill $(doker ps -q)

If we want to remove all container and images use `docker system prune -a` it will remove all stopped containe, network not used by at least one container, images without al least one container associat to them and all buil catche.
...
```
# when we pull the image from official image provided by dockerhub
docker pull imagename:tag/version

# when we pull the image from our repo
docker pull name_of_the_our_repo/imagename:tag/version
```
