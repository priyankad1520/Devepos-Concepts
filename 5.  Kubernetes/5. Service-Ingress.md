# Service
Service is a Kubernetes object that exposes a group of Pods using labels and selectors.  It provides a stable IP and DNS name to access Pod.

A Service selects multiple Pods using labels.
```Example:
...
selector: app=nginx
```
Now Service connects to all Pods having that label.
...
- “A Kubernetes Service exposes Pods, not directly to users always, but as a stable networking endpoint for internal or external communication.
- “A Kubernetes Service is used to expose a group of Pods and provide stable communication between applications inside or outside the cluster.”
- Kubernetes Service provides stable network access to Pods. Users usually access applications through LoadBalancer or Ingress, which forwards traffic to the Service, and the Service routes traffic to Pods
## Types of Services
1. ClusterIP: Default service type with internal IP. ClusterIP is used when one application inside Kubernetes wants to talk to another application inside Kubernetes.
- Example: Frontend Pod → Backend Service → Backend Pods OR Application → Database Service
- Why: For communication between Pods with pods inside cluster. 
...
- Service helps Pods communicate with other Pods.
- Worker nodes are just machines running Pods.

2. NodePort: Exposes service using Node IP + Port. NodePort exposes application outside the cluster using: NodeIP + Port
- Example: 192.168.1.10:30080. Now external users can access application.
- Flow: User Browser → Worker Node IP:Port → Service → Pods
...
| ----------------------- | ---------------------------- |
| Uses Node IP + Port     | Uses Public IP               |
| Manual access           | Easy internet access         |
| Basic external exposure | Production-grade exposure    |
| Mostly for testing/labs | Mostly for production        |
| Works everywhere        | Needs cloud provider support |
...
# Ingress:
Ingress provides a single entry point to control and manage external traffic by routing requests to Kubernetes Services.
It provides single entry point, controls external traffice by using routing traffic.
