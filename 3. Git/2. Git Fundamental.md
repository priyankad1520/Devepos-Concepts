...
### Commit
A Git commit saves changes in the repository along with information like time, date, author, and commit message. A Git commit saves changes only in theLocal Repository
- A commit is a saved snapshot of your project at a specific time. A commit is a snapshot of changes saved in the git repository along with a messange describing the changes.
- It helps track project history and allows reverting to previous version.
...
| Git Command                      | Explanation                      | Example                                                 | Why We Use It                  |
| -------------------------------- | -------------------------------- | ------------------------------------------------------- | ------------------------------ |
| `git --version`                  | Checks installed Git version     | `git --version`                                         | Verify Git installation        |
| `git config --global user.name`  | Sets Git username                | `git config --global user.name "Priyanka"`              | Adds username to commits       |
| `git config --global user.email` | Sets Git email                   | `git config --global user.email "abc@gmail.com"`        | Identifies commit owner        |
| `git config --list`              | Displays Git configuration       | `git config --list`                                     | Verify Git settings            |
| `git init`                       | Initializes new Git repository   | `git init`                                              | Start version control          |
| `git clone`                      | Copies repository from GitHub    | `git clone https://github.com/test/app.git`             | Download project locally       |
| `git status`                     | Shows file status                | `git status`                                            | Check modified/untracked files |
| `git add file.txt`               | Adds single file to staging      | `git add app.py`                                        | Prepare file for commit        |
| `git add .`                      | Adds all files to staging        | `git add .`                                             | Stage all changes              |
| `git commit -m`                  | Saves changes permanently        | `git commit -m "Added login feature"`                   | Create project snapshot        |
| `git log`                        | Shows commit history             | `git log`                                               | Track previous commits         |
| `git log --oneline`              | Compact commit history           | `git log --oneline`                                     | Quick history view             |
| `git diff`                       | Shows file changes               | `git diff`                                              | Compare modifications          |
| `git remote add origin`          | Connects local repo to GitHub    | `git remote add origin https://github.com/user/app.git` | Link GitHub repository         |
| `git remote -v`                  | Shows remote repository URLs     | `git remote -v`                                         | Verify remote connection       |
| `git push`                       | Uploads code to GitHub           | `git push origin main`                                  | Share code remotely            |
| `git pull`                       | Downloads latest changes         | `git pull origin main`                                  | Update local repository        |
| `git fetch`                      | Downloads changes without merge  | `git fetch`                                             | Check remote updates safely    |
| `git branch`                     | Lists branches                   | `git branch`                                            | View available branches        |
| `git branch branch-name`         | Creates new branch               | `git branch feature-login`                              | Work on new feature            |
| `git checkout branch-name`       | Switches branch                  | `git checkout feature-login`                            | Move between branches          |
| `git checkout -b`                | Creates and switches branch      | `git checkout -b dev`                                   | Faster branch creation         |
| `git merge`                      | Combines branches                | `git merge feature-login`                               | Merge feature into main        |
| `git branch -d`                  | Deletes branch                   | `git branch -d feature-login`                           | Remove unused branch           |
| `git stash`                      | Temporarily saves changes        | `git stash`                                             | Save unfinished work           |
| `git stash list`                 | Shows stashed changes            | `git stash list`                                        | View saved temporary work      |
| `git stash apply`                | Restores stashed changes         | `git stash apply`                                       | Continue previous work         |
| `git restore file.txt`           | Removes local changes            | `git restore app.py`                                    | Undo modifications             |
| `git restore --staged`           | Removes staged file              | `git restore --staged app.py`                           | Unstage file                   |
| `git rm`                         | Deletes file from Git            | `git rm old.txt`                                        | Remove tracked file            |
| `git mv`                         | Renames file                     | `git mv old.txt new.txt`                                | Rename tracked file            |
| `git reset --soft HEAD~1`        | Removes commit but keeps changes | `git reset --soft HEAD~1`                               | Undo commit safely             |
| `git reset --hard HEAD~1`        | Removes commit and changes       | `git reset --hard HEAD~1`                               | Completely rollback            |
| `git revert`                     | Reverts commit safely            | `git revert commit-id`                                  | Undo shared changes            |
| `git tag`                        | Creates release version          | `git tag v1.0`                                          | Mark application release       |
| `git push origin --tags`         | Pushes tags to GitHub            | `git push origin --tags`                                | Share release versions         |
| `git show`                       | Shows commit details             | `git show commit-id`                                    | Inspect commit changes         |
| `git reflog`                     | Shows all Git activities         | `git reflog`                                            | Recover deleted commits        |
| `git cherry-pick`                | Applies specific commit          | `git cherry-pick commit-id`                             | Copy commit to another branch  |
| `git rebase`                     | Reapplies commits cleanly        | `git rebase main`                                       | Cleaner commit history         |
| `git clean -f`                   | Removes untracked files          | `git clean -f`                                          | Clean workspace                |
| `git blame`                      | Shows who changed each line      | `git blame app.py`                                      | Track code ownership           |
| `git archive`                    | Creates zip/tar archive          | `git archive main --format=zip > app.zip`               | Export project                 |
| `git shortlog`                   | Shows commit summary by author   | `git shortlog`                                          | Team contribution tracking     |
| `git ls-files`                   | Lists tracked files              | `git ls-files`                                          | View Git-managed files         |
| `git cherry`                     | Shows unmerged commits           | `git cherry -v`                                         | Compare branches               |
| `git bisect`                     | Finds problematic commit         | `git bisect start`                                      | Debug issues faster            |
...
- This .git folder stores: Commit history, Branch information, Tracking changes, Git configuration, Version control data
